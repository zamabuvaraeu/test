...
FREEBASIC_PARAMETERS_BASE=-O 0 -r -gen $(CODE_GENERATION_BACKEND) -maxerr 1 -w all $(INCLUDEFILESPATH) $(UNICODE_FLAG) $(WITHOUT_CRITICAL_SECTIONS_FLAG) $(WINDOWS_SERVICE_FLAG) $(WITHOUT_RUNTIME_FLAG) $(GUIDS_WITHOUT_MINGW_FLAG) $(PERFORMANCE_TESTING_FLAG)
...
GCC_COMPILER_PARAMETERS=$(GCC_WARNING) $(GCC_NOINCLUDE) $(GCC_ARCHITECTURE) -masm=intel -S
FREEBASIC_PARAMETERS=$(FREEBASIC_PARAMETERS_BASE)
GCC_LINKER_PARAMETERS=-m $(PE_FILE_FORMAT) -subsystem $(EXETYPEKIND) -e $(ENTRY_POINT) $(DEFAULT_STACK_SIZE) -L $(COMPILER_LIB_PATH) -L "." --no-seh --nxcompat --gc-sections --print-gc-sections $(IMAGE_VERSION_MAJOR) $(IMAGE_VERSION_MINOR)
